...
#'   and the calculated ages: 'DNAmAgePanMammalianBlood2',
#'   'DNAmAgePanMammalianBlood3', 'DNAmRelativeAge_Blood', and 'DNAmRelativeAdultAge_Blood'.
#'
#' @export
#'
#' @references
...
#'
#' clock_results <- PanMammalianBlood(
...
PanMammalianBlood <- function(sample_info,
                              beta.m,
                              anage_data,
                              verbose = FALSE) {

  if (verbose) {
    print(paste0("[PanMammalianBlood] Starting Universal Pan-Mammalian Blood Clock calculation..."))
  }

  data("PanMammalianBloodCoef")
  ...
  age.name <- c('DNAmAgePanMammalianBlood2', 'DNAmAgePanMammalianBlood3')

  # --- 3. Process Input Data ---

  if (!all(c("SpeciesLatinName", "Sample") %in% names(sample_info))) {
    stop("[PanMammalianBlood] `sample_info` must contain 'SpeciesLatinName' and 'Sample' columns.")
  }
  if (is.null(rownames(beta.m)) || is.null(colnames(beta.m))) {
    stop("[PanMammalianBlood] `beta.m` must have rownames (CpG IDs) and colnames (Sample IDs).")
  }
  ...
  if (!all(samples_info %in% samples_beta)) {
    ...
    stop(paste("[PanMammalianBlood] The following samples are in `sample_info` but not in `beta.m` colnames:",
  ...
  anage <- anage_data
  dat.meth0 <- beta.m
  glmnet.list <- PanMammalianBloodCoef # Uses the object from the package's data
  ...
  if (verbose) {
    print(paste0(
      "[PanMammalianBlood] Number of represented PanMammalianBlood CpGs (max=",
  ...
  if (length(missing_cpgs) > 0) {
    ...
    # warning(paste("[PanMammalianBlood] The following required CpGs are missing from `beta.m` rownames and will be treated as having NA values:",
  ...
  output_cols <- c('Sample', 'SpeciesLatinName', 'MammalNumberHorvath', 'Age', 'Tissue',
                   'DNAmRelativeAge_Blood', 'DNAmRelativeAdultAge_Blood', age.name)
