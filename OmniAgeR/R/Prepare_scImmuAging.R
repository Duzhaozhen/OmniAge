...
#' @param input A Seurat object. Its metadata (meta.data) must contain "donor_id" and "age" columns.
#' @param cell_type A string specifying the cell type to be analyzed.
#' @param model A string or object specifying the model to be used.
...
scImmuAging_PreProcess <- function(input, cell_type, model, marker_gene)
{
  new.seurat = subset(input, features = marker_gene)
  preprocessed_df = scImmuAging_preprocessing(new.seurat) %>%
    dplyr::group_by(donor_id, age) %>%
    tidyr::nest()

  preprocessed_df <- preprocessed_df %>%
    dplyr::mutate(pseudocells = purrr::map(data, pseudocell))
...
scImmuAging_preprocessing <- function(seurat_object)
{
  ...
  input_mtx <- t(as.matrix(GetAssayData(seurat_object, assay = "RNA", layer = "data")))
  combined_input <- dplyr::as_tibble(cbind(meta_data, input_mtx))
...
#' @param input the output from scImmuAging_PreProcess() function.
...
pseudocell <- function(input, size=15, n=100, replace="dynamic") {
  ...
  return(dplyr::as_tibble(pseudocells))
}

#' predict age for each individual
#'
#' @param preprocessed_df the output from scImmuAging_PreProcess() function.
...
scImmuAgingCalculator <- function(preprocessed_df, model, marker_gene) {
...
{
  donor_df <- predict_res %>%
    # 使用 dplyr::group_by 和 dplyr::mutate
    dplyr::group_by(donor_id) %>%
    dplyr::mutate(predicted = round(mean(Prediction))) %>%
